...
class POSService {
  ...
  Future<Sale> recordSale(
    ...
    // Use a single transaction to ensure atomicity for both sale record and stock updates
  ...
  Future<List<SaleItem>?> retrieveHeldCart() async {
    ...
    if (snapshot != null) {
      ...
      final itemsMapList = (heldCartMap['items'] as List);
      final items = itemsMapList.map((itemMap) => SaleItem.fromMap(itemMap)).toList();
